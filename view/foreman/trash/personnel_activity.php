...
if ($job_status == "completed" || $job_status == "cancelled" || $job_status == "closed" || $job_status == "queued") {
	echo "<script> window.location.replace('?page=working_jobs')</script>";
}else{
...
<section class="panel">
			...
			<div class="col-md-12">
				...
				<div class="panel-body">
					<table class="table table-striped" id="datatable-task-per">
						...
						<tbody>
							<?php foreach($equipment_needed_list as $row): 
								if ($row['no_optr'] == "1") {
									$row['no_optr'] = "YES";
								}else{
									$row['no_optr'] = "NO";
								...
								<tr>
									...
									<td><?= $row['no_eqpt'] ?></td>
									<td><?= $row['no_optr'] ?></td>
									<td><button type="button" class="btn btn-default btn-sm" onclick="del_eqpt('<?= $row['id'] ?>')"><i class="fa fa-pencil"></i></button></td>
								</tr>
							<?php endforeach ?>
						</tbody>
					</table>
			...
			<div class="col-md-12">
				...
				<div class="panel-body">
					<table class="table table-bordered table-striped mb-none" id="datatable-def3">
						<thead>
							<tr>
								<th colspan="5" class="center"	style="background-color: #DADADA">Equipment</th>
							</tr>
							<tr>
								<th>No#</th>
								<th>Equipment</th>
	...
	<div class="modal fade" id="equipment_activity" role="dialog">
		<div class="modal-dialog">
				...
				<div class="modal-body">
					<table class="table table-bordered table-striped mb-none" id="datatable-task-per">									
						...
						<tbody>
							<?php 
							$i=1;
							foreach($equipment_list2 as $rows): 
								?>
								<tr class="gradeX">
									<td class="center"><button type="button" class="btn btn-info btn-xs" onclick="add_equipment('<?= $rows['id'] ?>')"><i class="fa fa-plus"></i></button></td>
									<td><?= $rows['eqpt_name'] ?></td>
									<td><input type="number" id="no_eqpt_<?= $rows['id'] ?>"></td>
									<td><select class="form-control populate" id="no_optr_<?= $rows['id'] ?>">

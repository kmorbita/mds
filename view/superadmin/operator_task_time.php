<?php 
		// get request number
if (isset($_GET['page']) && $_GET['page'] == "operator_task") {
	$id = $_GET['req'];
	$task = $_GET['task'];
	$tasks = $controller->operator_task_details($id,$task);
}
		// get request number
?>
<section class="panel">
	<header class="panel-heading">
		...
		<h2 class="panel-title">Operator Task Timestamp Details</h2>
	</header>
		...
		<table class="table table-bordered table-striped mb-none" id="datatable-operator">									
			...
			<tbody>
				...
				foreach($tasks as $job):
					?>
					<tr class="gradeX job_optr_task_<?= $job['id'] ?>">
						<td><?= $i ?></td>
						...
						<td><button class="btn btn-default btn-xs fa fa-pencil" onclick="edit_optr_task_time('<?= $job['id'] ?>')"></button>|<button class="btn btn-default btn-xs fa fa-trash-o" onclick="return confirm('Are you sure?')?delete_optr_task_time('<?= $job['id'] ?>'):'';"></button></td>
					</tr>
					<?php
					$i++;
				endforeach ?>
...
<div class="modal fade" id="edit_optr_task_time" role="dialog">
	<div class="modal-dialog">
		...
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">Edit Operator Task Timestamp</h4>
			</div>
			<div class="modal-body">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default btn-sm" id="update_optr_task_timestamp">Update</button>

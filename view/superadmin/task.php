			...
			<tbody>
				...
				foreach ($task as $row):?>
					<tr class="task_<?= $row['id']?>">
						...
						<td><?= $row['task'] ?></td>
						<td><?= $row['task_for'] ?></td>
						<td><a href="?page=edittask&task_id=<?= $row['id']?>&req=<?= $id ?>"><button type="button" class="btn btn-info btn-sm"><i class="fa fa-pencil"></i></button></a>|<button type="button" class="btn btn-danger btn-sm" onclick="del_task('<?= $row['id'] ?>')"><i class="fa fa-trash-o"></i></button></td>
					</tr>
					<?php $i++; 
				endforeach ?>
			</tbody>

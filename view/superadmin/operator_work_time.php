<?php 
		// get request number
if (isset($_GET['page']) && $_GET['page'] == "operator_work_time") {
	$request_no = $_GET['req'];
	$emp_id = $_GET['emp_id'];
	$operator_timestamp = $controller->operator_act($request_no,$emp_id);
	$operator_info = $controller->operator_info($request_no,$emp_id);
...
<section class="panel">
	<header class="panel-heading">
		<div class="panel-actions">
			<button type="button" class="btn btn=sm btn-default" data-toggle="modal" data-target="#add_optr_timestamp">Add Operator Timestamp</button>
			<a href="#" class="fa fa-caret-down"></a>
			<!-- <a href="#" class="fa fa-times"></a> -->
		</div>

		<h2 class="panel-title">Edit Operator Job Activity Timestamp</h2>
	</header>
	<div class="panel-body">
		...
		<div class="row">
			<div class="col-md-6">
				<div class="form-group">
					...
					<input type="text" class="form-control" value="<?= $operator_info['emp_id']  ?>" id="per_id" readonly>
				</div>
			</div>
			<div class="col-md-6">
				<div class="form-group">
					<label>Personnel Name</label>
					<input type="text" class="form-control" value="<?= $operator_info['fname']." ".$operator_info['mname']." ".$operator_info['lname']  ?>" id="per_name" readonly>
				</div>
			</div>
		</div>
		<!-- </div> -->
		<table class="table table-bordered table-striped mb-none" id="datatable-operator">									
			...
			<tbody>
				<?php 
				$i=1;
				foreach($operator_timestamp as $job):
					?>
					<tr class="gradeX job_optr_<?= $job['id'] ?>">
						<td><?= $i ?></td>
						...
						<td><?= $job['reason'] ?></td>
						<td><button class="btn btn-default btn-xs fa fa-pencil" onclick="edit_job_optr_time('<?= $job['id'] ?>')"></button>|<button class="btn btn-default btn-xs fa fa-trash-o" onclick="return confirm('Are you sure?')?delete_job_optr_time('<?= $job['id'] ?>'):'';"></button></td>
					</tr>
					<?php
					$i++;
				endforeach ?>
...
<div class="modal fade" id="edit_job_optr_timestamp" role="dialog">
	<div class="modal-dialog">
		...
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">Edit Job Operator Timestamp</h4>
			</div>
			<div class="modal-body">
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" id="update_work_paused"><b><span class="color-text" id="update_work_stopped_error"></span></b>
				</div>
				<div class="form-group">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default btn-sm" id="update_job_optr_timestamp">Update</button>
...
<div class="modal fade" id="add_optr_timestamp" role="dialog">
	<div class="modal-dialog">
		...
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
				<h4 class="modal-title">Add Operator Timestamp</h4>
			</div>
			<div class="modal-body">
				<div class="form-group">
					<label>Status</label>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default btn-sm" id="insert_optr_timestamp">Add</button>
